...
def draw_and_save(Q, R, T, a1, a2, a3, a4, b1, b2, b3, b4, m, output_dir, r, robustness, save_data, t_left1, t_left2,
                  t_right1, t_right2, u, u_max, x, y, operator):
    t = np.arange(0, T + 1)
    u = u.X.flatten()
    x = x[:, 1].X.flatten()
    y = y.X.flatten()
    ...
    if operator[0] == 'G':
        ...
        color = 'skyblue'
        label = 'Eventually'
    axs[y_position].fill_between(np.arange(t_left1, t_right1 + 1), b1 / a1, b3 / a3, color=color, alpha=0.5,
                                 label=label)
    if operator[-1] == 'G':
        color = 'springgreen'
        label = 'Globally'
        ...
        color = 'skyblue'
        label = 'Eventually'
    axs[y_position].fill_between(np.arange(t_left2, t_right2 + 1), b4 / a4, b2 / a2, color=color, alpha=0.5,
                                 label=label)
    ...
    axs[y_position].axhline(y=b3 / a3, color='gray', linestyle='--', alpha=0.5)
    axs[y_position].axhline(y=b4 / a4, color='gray', linestyle='--', alpha=0.5)
    axs[y_position].axhline(y=0, color='gray', linestyle='--', alpha=0.5)
    axs[y_position].legend(loc='lower left')
    # # plot u
    # u_position = 3
    ...
    rhos3 = cal_shifted_rho(a3, b3, t_left1, t_right1, y, operator[1])
    rhos4 = cal_shifted_rho(a4, b4, t_left2, t_right2, y, operator[3])
    l = min(len(rhos1), len(rhos2), len(rhos3), len(rhos4))
    rhos = np.min(np.stack([rhos1[:l], rhos2[:l], rhos3[:l], rhos4[:l]], 0), 0)  # conjunction: min, disjunction: max
    ...
    xi_plus, xi_minus, rho, theta = draw_robustness_graph(axs[robustness_position], xi_minus, t_xi_minus, y_xi_minus, xi_plus, t_xi_plus, y_xi_plus,
                          x_lim_xi)
    if save_data:
